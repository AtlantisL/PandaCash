                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zPANDA ought to be enough for anybody." - Bill Gates, 2017
    ui->zPANDApayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzPANDASyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzPANDA->setEnabled(false);
        ui->pushButtonMintzPANDA->setToolTip(tr("zPANDA is currently disabled due to maintenance."));

        ui->pushButtonSpendzPANDA->setEnabled(false);
        ui->pushButtonSpendzPANDA->setToolTip(tr("zPANDA is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zPANDApayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzPANDA_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zPANDA is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zPANDA...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zPANDA in ") + 
...
void PrivacyDialog::on_pushButtonSpendzPANDA_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zPANDA is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zPANDA
        sendzPANDA();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zPANDA
    sendzPANDA();
}

void PrivacyDialog::on_pushButtonZPANDAControl_clicked()
{
    ZPANDAControlDialog* zPANDAControl = new ZPANDAControlDialog(this);
    zPANDAControl->setModel(walletModel);
    zPANDAControl->exec();
}

void PrivacyDialog::setZPANDAControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzPANDASelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzPANDA()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid PandaCash Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zPANDApayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zPANDA is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " PANDA </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zPANDA</b>";
    ...
    // use mints from zPANDA selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZPANDAControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZPANDAControlDialog::GetSelectedMints();
    }
    ...
    // Clear zpanda selector in case it was used
    ZPANDAControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zPANDA Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " PANDA, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zPANDA Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zPANDA </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zPANDA "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zPANDA "));
    ui->labelzPANDAAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzPANDASyncStatus->setVisible(fShow);
}
