// Copyright (c) 2017 The PandaCash developers
// Distributed under the MIT/X11 software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include "zpandacontroldialog.h"
#include "ui_zpandacontroldialog.h"
...
std::list<std::string> ZPANDAControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZPANDAControlDialog::listMints;

ZPANDAControlDialog::ZPANDAControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZPANDAControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZPANDAControlDialog::~ZPANDAControlDialog()
{
    delete ui;
}

void ZPANDAControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZPANDAControlDialog::updateList()
{
...
void ZPANDAControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZPANDAControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZPANDA_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZPANDAControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZPANDAControlDialog::GetSelectedMints()
{
...
void ZPANDAControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
