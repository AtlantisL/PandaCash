...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 PANDA at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zPANDA send
enum ZerocoinSpendStatus {
    ZPANDA_SPEND_OKAY = 0,                            // No error
    ZPANDA_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZPANDA_WALLET_LOCKED = 2,                         // Wallet was locked
    ZPANDA_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZPANDA_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZPANDA_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZPANDA_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZPANDA_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZPANDA_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZPANDA_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZPANDA_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZPANDA_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZPANDA_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZPANDA_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZPANDA_SPENT_USED_ZPANDA = 14                       // Coin has already been spend
};
...
public:
    ...
    */
    ...
    void setZPANDAAutoBackups(bool fEnabled)
    {
...
public:
    ...
    CAmount GetLockedCredit() const
    {
        ...
        for (unsigned int i = 0; i < vout.size(); i++) {
            ...
            if (fMasterNode && vout[i].nValue == 200000 * COIN) {
                nCredit += pwallet->GetCredit(txout, ISMINE_SPENDABLE);
            }
