...
#if defined(NDEBUG)
#error "PandaCash cannot be compiled without assertions."
#endif
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zPANDA spend tx %s already in block %d", tx.GetHash().GetHex(), nHeightTx),
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
                    return state.Invalid(error("%s : zPANDA spend with serial %s is already in block %d\n",
                ...
                if (!spend.HasValidSerial(Params().Zerocoin_Params()))
                    return state.Invalid(error("%s : zPANDA spend with serial %s from tx %s is not in valid range\n",
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight == 0) {
		...
		nSubsidy = 900 * COIN;
    } else if (nHeight > 100000 && nHeight <= 600000) {
		nSubsidy = 1000 * COIN;
    } else if (nHeight > 600000 && nHeight <= 1200000) {
		nSubsidy = 800 * COIN;
    } else if (nHeight > 1200000 && nHeight <= 1500000) {
		nSubsidy = 750 * COIN;
    } else if (nHeight > 1500000 && nHeight <= 1800000) {
	    nSubsidy = 700 * COIN;
    } else if (nHeight > 1800000 && nHeight <= 2100000) {
	    nSubsidy = 650 * COIN;
    } else if (nHeight > 2100000 && nHeight <= 2400000) {
	    nSubsidy = 600 * COIN;
    } else if (nHeight > 2400000 && nHeight <= 2700000) {
	    nSubsidy = 550 * COIN;
    } else if (nHeight > 2700000 && nHeight <= 3000000) {
	    nSubsidy = 500 * COIN;
    } else if (nHeight > 3000000 && nHeight <= 3300000) {
	    nSubsidy = 450 * COIN;
    } else if (nHeight > 3300000 && nHeight <= 3600000) {
	    nSubsidy = 400 * COIN;
    } else if (nHeight > 3600000 && nHeight <= 3900000) {
	    nSubsidy = 350 * COIN;
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount)
{
    ...
    if (nHeight > 100000 && nHeight <= 1200000) {
        blockValue - (blockValue * 35 / 100);
    }
	
	if (nHeight >= 103735 && nHeight <= 1200000)
	{
		return blockValue - (blockValue * 35 / 100);
	}
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from PandaCash
...
{
    RenameThread("pandacash-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZPANDAMinted()
{
...
void RecalculateZPANDASpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zPANDA supply
...
bool RecalculatePANDASupply(int nHeightStart)
{
                ...
                if (zerocoinDB->ReadCoinSpend(spend.getCoinSerialNumber(), hashTxFromDB)) {
                    if(IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTxSpend)) {
                        if(!fVerifyingBlocks || (fVerifyingBlocks && pindex->nHeight > nHeightTxSpend))
                            return state.DoS(100, error("%s : zPANDA with serial %s is already in the block %d\n",
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zPANDASpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert PANDA to zPANDA
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zPANDA spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zPANDA serial %s in block\n Block: %s",
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zPANDA mints and %d zPANDA spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // PandaCash: We use certain sporks during IBD, so check to see if they are
